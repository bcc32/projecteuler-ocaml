...
module Chance_card = struct
  ...
  let dist =
    D.uniform'
      [ No_op; No_op; No_op; No_op; No_op; No_op
      ; Advance_to_go
      ; Go_to_jail
      ; Go_to_C1
      ; Go_to_E3
...
module Square : sig
  ...
  module T = struct
    type t =
      ...
      | Jail
      | Free_parking
      | A of int | B of int | C of int | D of int
      | E of int | F of int | G of int | H of int
      | R of int | T of int | U of int
    [@@deriving bin_io, compare, sexp]

    let all =
      [ Go; A 1; Community_chest 1; A 2; T 1; R 1; B 1; Chance 1; B 2; B 3
      ; Jail; C 1; U 1; C 2; C 3; R 2; D 1; Community_chest 2; D 2; D 3
      ; Free_parking; E 1; Chance 2; E 2; E 3; R 3; F 1; F 2; U 2; F 3
      ; Go_to_jail; G 1; G 2; Community_chest 3; G 3; R 4; Chance 3; H 1; T 2; H 2 ]
    ;;
  end
  include T
  ...
  let is_chance          = function Chance _          -> true | _ -> false
  let is_community_chest = function Community_chest _ -> true | _ -> false
  let is_railway_company = function R _               -> true | _ -> false
  let is_utility_company = function U _               -> true | _ -> false
...
module Game_state_dist : sig
  ...
  let flip_card list ~p_flip =
    let rotated =
      ...
      | hd :: tl -> tl @ [hd]
    in
    List.map2_exn list rotated ~f:(fun x y ->
      D.combine ~d1:y ~d2:x ~p1:p_flip)
  ;;
...
module M = struct
  ...
  let main () =
    ...
    let die =
      ...
      let open D.Let_syntax in
      let%map a = one_die and b = one_die in
      a + b
    in
    for _ = 1 to 10000 do
