...
(* Each coin flip, all possible starting values [x] result in two equally likely
   outcomes: [x * (1 + 2 * f)] and [x * (1 - f)].

   The distribution for each step evolves over time like so (let [a = 1 - f] and
   [b = 1 + 2 * f]):
   ...
   determine the probability [Pr(money >= 10^9 at 1000 flips)] as a function of
   [f] rather straightforwardly. *)
...
(* incomplete *)
include (val Solution.make ~problem:(Number 267) ~main)
