...
let g a n b m =
  Number_theory.Int.(
    ...
    then 0
      ...
      let (y, modulus) = chinese_remainder_theorem
                           [ (a / x, n / x)
                           ; (b / x, m / x) ]
      in
      (y % modulus) * x + a % x))
...
let%test_unit _ =
  ...
  let gen =
    ...
    let%bind n = Int.gen_incl 1 limit and m = Int.gen_incl 1 limit in
    let%map a = Int.gen_incl 0 (n - 1) and b = Int.gen_incl 0 (m - 1) in
    (a, n, b, m)
  in
  Quickcheck.test gen ~sexp_of:[%sexp_of: int * int * int * int]
    ~f:(fun (a, n, b, m) ->
      let lowest =
