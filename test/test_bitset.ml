    ...
    struct
      ...
      module Test = struct
        ...
        let quickcheck_shrinker =
          Quickcheck.Shrinker.create (fun (start, _) ->
            Quickcheck.Shrinker.shrink [%quickcheck.shrinker: Elt.t Set.t] (to_set start)
      ...
      let%test_unit "sexp_of_t" =
        Base_quickcheck.Test.run_exn
          ...
          ~f:(fun (t, set) ->
            [%test_result: Sexp.t] ([%sexp_of: t] t) ~expect:([%sexp_of: Int.Set.t] set))
      ;;
        ...
        Base_quickcheck.Test.run_exn
          ...
          ~f:(fun sexp ->
            [%test_result: Int.Set.t option]
              (Option.map ~f:to_set (Option.try_with (fun () -> [%of_sexp: t] sexp)))
      ...
      let%test_unit "add" =
        Base_quickcheck.Test.run_exn
          ...
          ~f:(fun ((t, set), elt) ->
            [%test_result: Int.Set.t] (to_set (add t elt)) ~expect:(Set.add set elt))
      ;;
      ...
      let%test_unit "remove" =
        Base_quickcheck.Test.run_exn
          ...
          ~f:(fun ((t, set), elt) ->
            [%test_result: Int.Set.t] (to_set (remove t elt)) ~expect:(Set.remove set elt))
      ;;
      ...
      let%test_unit "union" =
        Base_quickcheck.Test.run_exn
          ...
          ~f:(fun ((t1, set1), (t2, set2)) ->
            [%test_result: Int.Set.t] (to_set (union t1 t2)) ~expect:(Set.union set1 set2))
      ;;
      ...
      let%test_unit "inter" =
        Base_quickcheck.Test.run_exn
          ...
          ~f:(fun ((t1, set1), (t2, set2)) ->
            [%test_result: Int.Set.t] (to_set (inter t1 t2)) ~expect:(Set.inter set1 set2))
      ;;
      ...
      let%test_unit "diff" =
        Base_quickcheck.Test.run_exn
          ...
          ~f:(fun ((t1, set1), (t2, set2)) ->
            [%test_result: Int.Set.t] (to_set (diff t1 t2)) ~expect:(Set.diff set1 set2))
      ;;
      ...
      let%test_unit "fold" =
        Base_quickcheck.Test.run_exn
          ...
          ~f:(fun (t, set) ->
            ...
            [%test_result: int list] (fold t ~init ~f) ~expect:(Set.fold set ~init ~f))
      ;;
      ...
      let%test_unit "length" =
        Base_quickcheck.Test.run_exn
          ...
          ~f:(fun (t, set) -> [%test_result: int] (length t) ~expect:(Set.length set))
      ;;
